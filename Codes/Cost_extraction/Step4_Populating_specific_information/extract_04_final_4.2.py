...
OUT_XLSX = r".../extraction_final_4.2.xlsx"   # Path of output results
...
1. population_age:
• Format rules:
  > Single age: X years (e.g., 45 years);
  > Age range: X-Y years (e.g., 18-65 years);
  > No upper limit: X years and above (e.g., 65 years and above);
  > No lower limit: 0-Y years (e.g., 0-5 years).
• Not age-specific or covers all ages → NR.
• NEVER convert non-standard descriptions to age ranges.

2. population_gender:
• ONLY these exact values allowed:
  > Male (male-only populations);
  > Female (female-only populations);
  > NR (mixed gender or not specified).
• NEVER use variations like M, F, men, women.

3. population_income:
• Extract EXACT income description as stated in source.
• No income stratification or covers all incomes → NR.
• Preserve original terminology (e.g., low-income, Medicaid beneficiaries).

4. population_other:
• ONLY include other population characteristics.
• If cost comes from experiment/cohort → Brief description of experiment group/specific cohort.
...
Output format:
Combine the above information along with the literature_doi, totaling 5 items, to form specific information entry corresponding to each disease cost value. Format the output as JSON: {{population_age ..., population_gender ..., population_income …, population_other ..., literature_doi ...}}.
For example:
```json
{{
“population_age”: “18-65 years”,
“population_gender”: “Male”,
“population_income”: “Low-income population”,
“population_other”: “NR”,
