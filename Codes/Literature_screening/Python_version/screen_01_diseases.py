...
merged_files = sorted(
    glob.glob(os.path.join(source_path, "original_literature_*.xls"))
)

if not merged_files:
    raise FileNotFoundError("original_literature_*.xls not found")
...
final_df.to_csv(
    os.path.join(result_dir, "parsed_results_clean_1st_step.csv"),
    index=False,
    encoding="utf-8-sig"
)
...
included.to_csv(
    os.path.join(output_dir, "included_only_1st_step.csv"),
    index=False,
    encoding="utf-8-sig"
)
unclear.to_csv(
    os.path.join(output_dir, "unclear_only_1st_step.csv"),
    index=False,
    encoding="utf-8-sig"
)
